...
#bebebe, -35px -35px 70px #ffffff; " class="w-full  p-5 mt-5 bg-gray-100">
        <section class="list__accounts w-full">
            ...
            <form action="index.php?user_id=<?= $user_id ?>" method="post">
                <h3 class="text-sm font-medium text-indigo-500 my-5">Người dùng : <?= $items[0]['name'] ?></h3>
                    ...
                    <table class="w-full text-center rounded-md shadow-md my-3">
                        ...
                        <tbody>
                            ...
                            foreach ($items as $item) {
                                extract($item);
                                $images = select_image_by_post_id($post_id);
                                $total = count_image_post($post_id);
                            ?>
                            <tr class="border-t-2 border-dashed" rowspan="<?= $total ?>">
                                <td class="p-2 whitespace-nowrap">
                                    <input type="checkbox" name="post_id[]" value="<?= $post_id ?>" class="checkbox" />
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                ...
                                <td class="p-2 whitespace-nowrap">
                                    ...
                                    <?php if (strpos($img['image'], '.') == false || $img['image'] == "") { ?>
                                    <p class="text-xs text-gray-900 mt-1 flex justify-center">
                                        Không có ảnh
                                    </p>
                                    <?php } else { ?>
                                    <img width="50" height="50"
                                        src="<?= $CONTENT_URL ?>/images/posts/<?= $img['image'] ?>"
                                        class="object-cover object-center rounded-sm" alt="">
                                    <?php } ?>
                                ...
                                <td class="p-2 whitespace-nowrap">
                                    <?php if ($status == 1) { ?>
                                    <span class="text-green-500 text-xs">Đã duyệt</span>
                                    <?php } else { ?>
                                    <span class="text-red-500 text-xs">Chưa duyệt</span>
                                    <?php } ?>
                                </td>
                                <td class="px-2 mt-7 whitespace-nowrap ">
                                    <a href="index.php?btn_delete&post_id=<?= $post_id ?>&user_id=<?= $user_id ?>"
                                        class="text-indigo-600 hover:text-indigo-900 flex items-center justify-center "><svg
...
1.5px 1.5px 2.5px #babecc, -2px -2px 5px #fff;" id="select"
                        class=" p-3  bpost   text-center rounded-md text-xs hover:bg-gray-200 leading-4 ">
                        Chọn tất cả
                    </label>
                    <label for="checkAll" style="text-shadow: 0.6px 0.6px 0 #fff; color: #61677c; box-shadow:
1.5px 1.5px 2.5px #babecc, -2px -2px 5px #fff;" id="unselect"
                        class=" p-3  bpost rounded-md  text-center text-xs hover:bg-gray-200 leading-4 ">
                        Bỏ chọn tất cả
                    </label>
